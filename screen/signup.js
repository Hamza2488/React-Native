...
const SignUp = ({navigation}) => {
  ...
  const signUpUser = ()=>{
    ...
    .then((res) => {
      ...
      .set(Model)
      .then(()=>{
        navigation.navigate('Home', res.user.userName)
        setLoader(false)
      })
  ...
  return (
    ...
    <View style={[styles.bgPrimary,{width:'100%',height:'100%'}]}>
      ...
      <View style={{backgroundColor:'white',width:'100%',height:'100%',borderTopRightRadius:50,borderTopLeftRadius:50,position:'relative',top:-70}}>
        ...
        <TextInput onChangeText={(e)=>setModel({...Model, userName:e})} style={{borderBottomWidth:2,borderBottomColor:'black',fontSize:15,marginVertical:15}}  placeholder='User Name' />
        ...
        <Button title={loader? 'Loading...': 'Sign Up'} onPress={signUpUser} color='#38b000'/>
        <View style={{flexDirection:'row', marginVertical:20,marginHorizontal:90}}>
          ...
          <Text style={[styles.mb3,styles.textPrimary,{fontWeight:'bold',fontSize:15,}]}>   Login</Text>
        </View>
        </View>
      </View>
    </View>
